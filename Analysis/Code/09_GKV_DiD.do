...
* Open log file 
local date "$date"
log using "$logs_analysis/`date'_analysis_gkv_aggregate_regs.log", text replace	


* ============================================================================ *
* Section 1: Above and Below vs Comparison
* ============================================================================ *
...
foreach cut in avg med {
	...
	* ------------------------------------------------------------------------ *
	foreach yvar in insured_lead_ln /*insured_change_perc*/ marketshare_change 		///
		...
		* Add overall DiD coefficients
		reghdfe `yvar' below above did_below did_above insured_initial_ln if year<2019, absorb(year type)
		local b_below = trim("`: di %10.3fc _b[did_below]'")
		local se_below = trim("`: di %10.3fc _se[did_below]'")
		local b_above = trim("`: di %10.3fc _b[did_above]'")
		local se_above = trim("`: di %10.3fc _se[did_above]'")
		...
		* DiD over years	
		...
		// Regression
		reghdfe `yvar' below above d_* insured_initial_ln if year<2019, absorb(type)
		...
		* Plot graph
		local date $date
		graph twoway ///
			(connected ball_above year, msize(medsmall) lcolor(orange) mcolor(orange) lpattern(solid) lwidth(medthin)) ///
			...
			text(`ymin_text' 2016.5 "ß_above = `b_above' (`se_above')", size(medsmall)) ///
			ytitle("`ytext'")
			
		graph export "$figures/DiD/Provider/`date'_DiD_Provider_`cut'_`yvar'_V2.pdf", replace
		graph close
