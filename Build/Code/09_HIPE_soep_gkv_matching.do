...
/* 	DATA INPUT
	$data_final/soep_merge.dta
	$data_final/GKV_aggregate.dta

	DATA INTERMEDIATE
	$data_temp/temp.dta
	$data_temp/temp_gkv_check.csv

	DATA OUTPUT
	$data_final/soep_gkv_match.dta
	
	DO-FILE OUTLINE
	1) SOEP data preparation
	2) Match GKV aggregate data 
	3) Adjust variables and create additional variables
	4) Complete missing GKV information
	5) Save data
*/
...
* Open log file 
local date "$date"
log using "$logs_build/`date'_HIPE_soep_gkv_matching.log", text replace		
...
* ---------------------------------------------------------------------------- *
	...
	* Load the merged soep data set
	use $data_final/soep_merge.dta, clear

	* Set id and time variable
	xtset pid syear
	
	* 1.1) Keep relevent person-year observations only 
	* ........................................................................ *
		...
		* Create "hi_switch" variable
			...
			* Overview amount of switches per year
				...
				table syear hi_switch if provider_l!="IKK/BIG" & provider_l!="LKK" & provider_l!="BKK" ///
					& provider_l!="PKV" & provider_l!="Other" & provider_l!="AOK unattributable" ///
...
* ---------------------------------------------------------------------------- *
	...
	* ........................................................................ *
	merge m:1 provider year using "$data_final/GKV_aggregate.dta"
	...
	* Prepare essential GKV aggregate data
	preserve 
		...
		* Load GKV_aggregate data
		use "$data_final/GKV_aggregate.dta", clear
		...
		* Keep relevant variables only
		keep provider type year addon addon_change addon_diff_avg addon_diff_predicted ///
			...
			capital_reserves_pc_ln capital_reserves_pc expenditure_admin_pc revenue_fund_pc_ln revenue_addon_pc_ln expenditure_admin_pc_ln ///
			rating rating_relative income_pc_ln members_ln admin_pc_change
		...
		* save temp version for match 
		save "$data_temp/temp.dta", replace
	...
	* ........................................................................ *
		...
		* Perform actual match
		merge m:1 provider_l year using "$data_temp/temp.dta"
	...
	* Type/provider
		...
		* Label define 
		...
		preserve
			...
			export delimited "$data_temp/temp_gkv_check.csv", replace delim(";")	
		restore
...
* 4) Identify observations with provider information 
* ---------------------------------------------------------------------------- *

	* Switches with provider information

	/* XX check if we can improve 

	- AOK Saarland and AOK Rheinland-Pfalz merged in 2013 -> can improve? 
	- AOK Nordwest = AOK Schleswig-Holstein + AOK Westfalen-Lippe 
	- AOK Plus = AOK Sachsen + AOK Thüringen 
	*/

	* Attributable information on lagged/current provider
	foreach var in info_l info {
		
		//cap drop `var'
		
		if "`var'"=="info"		local z provider
		if "`var'"=="info_l"	local z provider_l 

		gen `var'=1 if `z'!="IKK/BIG" & `z'!="LKK" & `z'!="BKK" ///
		& `z'!="PKV" & `z'!="Other" & `z'!="AOK unattributable" 	///
		& `z'!="AOK Saarland" & `z'!="AOK Rheinland-Pfalz" & `z'!="AOK Schleswig-Holstein" 	///
		& `z'!="AOK Sachsen" & `z'!="AOK Thueringen"
	}

	label var info 		"Insurer (current) information can be matched"
	label var info_l 	"Insurer (lagged) information can be matched"
...
* ---------------------------------------------------------------------------- *
	...
	* Save
	save "$data_final/soep_gkv_match.dta", replace
